...
import { ApplicationFormData, Session } from '@/types';
...
function ApplyFormContent() {
  ...
  // Initialize selected session from URL query
  useEffect(() => {
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      if (!formData.birthDate.trim()) {
        errs.birthDate = '생년월일을 입력해주세요. (예: 1995.06.15)';
      }
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-purple-100 p-6 sm:p-10 text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-neutral-600">
              담당 매니저가 서류 확인 후 24시간 이내에 개별 카카오톡으로 안내드립니다.
            </p>
          </div>
